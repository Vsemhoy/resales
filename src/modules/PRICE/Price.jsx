...
const Price = () => {
    ...
    useEffect(() => {
        const get_fields = async () => {
            ...
            try {
                setLoading(true);
                let fields_req = await PROD_AXIOS_INSTANCE.post(
                    `api/sales/price`,
                    format_data,
                );
                if (fields_req) {
    ...
    const handleExport = () => {
        ...
        const rows = selectedModels.map((m) => {
            ...
            addIfChecked(
                "Предпродажа",
                `${currentCurrency ? convertToRub(m.prices.price_0, m.currency) : m.prices.price_0} ${getCurrencySimbol(m.currency)}`,
            );
            addIfChecked(
                "Проектная",
                `${currentCurrency ? convertToRub(m.prices.price_10, m.currency) : m.prices.price_10} ${getCurrencySimbol(m.currency)}`,
            );
            addIfChecked(
                "Прайс 20",
                `${currentCurrency ? convertToRub(m.prices.price_20, m.currency) : m.prices.price_20} ${getCurrencySimbol(m.currency)}`,
            );
            addIfChecked(
                "Прайс 30",
                `${currentCurrency ? convertToRub(m.prices.price_30, m.currency) : m.prices.price_30} ${getCurrencySimbol(m.currency)}`,
            );
