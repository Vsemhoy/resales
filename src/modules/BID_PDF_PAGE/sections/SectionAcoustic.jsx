...
const emptyImage = (desc = '') => ({
  id: uuid(), file: null, description: desc, shrinkHeight: false, cropToCenter: false,
})
...
const emptyStiSection = (key) => ({
  enabled: false,
  page1: { text: ACOUSTIC_TEMPLATES[key]?.page1Text || '', image: { file: null, description: ACOUSTIC_TEMPLATES[key]?.page1ImageDescription || '' } },
  page2: { image: { file: null, description: ACOUSTIC_TEMPLATES[key]?.page2ImageDescription || '' }, text: ACOUSTIC_TEMPLATES[key]?.page2Text || '' },
})
const emptyRoom = () => ({
  id: uuid(), name: ACOUSTIC_TEMPLATES.roomName,
...
const MAX_ROOMS = 30, MAX_IMAGES = 10
...
export default function SectionAcoustic({ data, onChange, draftId, companyId }) {
  ...
  const ac = data.acousticCalculation || {
    intro: ACOUSTIC_TEMPLATES.intro, rooms: [],
  ...
  // Refs для скролла к якорям
  const scrollRef   = useRef(null)  // контейнер скролла
  const introRef    = useRef(null)
  const conclusionRef = useRef(null)
  const roomRefs    = useRef({})    // { [roomId]: el }
  ...
  const handleNavRoom = (id) => {
    setActiveRoomId(id)
    scrollTo(roomRefs.current[id])
  }

  return (
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>

      {/* ── Левая: контент (скроллится) ────────────────────────────────────── */}
        ...
        <div style={{
          ...
          <div style={{ padding: '4px 8px', borderTop: '1px solid #e8e8e8', borderBottom: '1px solid #e8e8e8' }}>
            <Button
              ...
              onClick={addRoom}
              disabled={(ac.rooms || []).length >= MAX_ROOMS}
              style={{ fontSize: 11 }}
            >
              Помещение
            </Button>
          ...
          <NavItem
            label="Заключение"
            sub
            dimmed={!ac.finalConclusion?.enabled}
    ...
    <div style={{ border: `1.5px solid ${accent}30`, borderRadius: 10, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: accent + '0a' }}>
        ...
        <Button size="small" danger icon={<DeleteOutlined />}
          onClick={() => { if (confirm(`Удалить «${room.name || 'помещение'}»?`)) onRemove() }}
        >Удалить</Button>
      </div>
      ...
      <div style={{ padding: '12px 14px' }}>
        ...
        {Object.entries(SUBSECTION_LABELS).map(([key, label]) => (
          <ImageTextSection key={key} label={label} data={room[key]}
            ...
            draftId={draftId} roomId={room.id} sectionKey={key}
            figureMap={figureMap} accent={accent}
          />
        ))}
    ...
    <SubBlock label={label} enabled={data?.enabled} onToggle={v => onChange({ enabled: v })} accent={accent}>
      <div style={{ marginBottom: 10 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          {images.length < MAX_IMAGES && <Button size="small" onClick={addImage} style={{ alignSelf: 'flex-start' }}>+ Картинка</Button>}
        </div>
      </div>
      <div>
    ...
    <SubBlock label={label} enabled={data?.enabled} onToggle={v => onChange({ enabled: v })} accent={accent}>
      ...
      <div style={{ marginTop: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          <Input size="small" placeholder="Название картинки..." value={data?.page1?.image?.description || ''} onChange={e => setPageImage('page1', { description: e.target.value })} />
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 8 }}>
        ...
        <Input size="small" placeholder="Название картинки..." value={data?.page2?.image?.description || ''} onChange={e => setPageImage('page2', { description: e.target.value })} />
      </div>
