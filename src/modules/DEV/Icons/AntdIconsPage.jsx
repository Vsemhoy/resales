...
const isIconComponent = (icon) => {
    ...
    if (typeof icon['render'] === 'function') { return true;}
    return false;
};
...
const allIcons = Object.entries(AntdIcons).filter(([name, Icon]) =>
    ...
    if (iconToFind.trim()) {
      setFilteredIcons(
        allIcons.filter(([name]) =>
          name.toLowerCase().includes(iconToFind.toLowerCase())
        )
      );
    } else {
    ...
    <div style={{background: 'white'}}>
      ...
      >
        {filteredIcons.length > 0 ? (
          filteredIcons.filter(([name, Icon])=>name.endsWith(type)).map(([name, Icon]) => (
            <div
              key={name}
