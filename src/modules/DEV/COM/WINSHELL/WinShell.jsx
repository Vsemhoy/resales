...
const WinShell = ({ children, title = 'Чатик', on_close }) => {
	...
	const snapMenu = [
		{
			key: 'snap-left',
			label: <div onClick={() => snapToSide('left')}>Snap Left</div>,
			icon: <ArrowLeftOutlined />,
		},
		{
			key: 'snap-right',
			label: <div onClick={() => snapToSide('right')}>Snap Right</div>,
			icon: <ArrowRightOutlined />,
		},
		{
			key: 'snap-bottom',
			label: <div onClick={() => snapToSide('bottom')}>Snap Bottom</div>,
			icon: <ArrowDownOutlined />,
		},
		{
			key: 'snap-off',
			label: <div onClick={() => snapToSide('none')}>Snap Off</div>,
	...
	return (
		...
		>
			...
			{!isCollapsed && snapSide === 'none' && (
				<div ref={resizeRef} className="resizer" onMouseDown={startResize} />
			)}
