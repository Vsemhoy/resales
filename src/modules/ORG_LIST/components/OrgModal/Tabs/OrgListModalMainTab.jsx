...
const OrgListMainTab = (props) => {
	...
	useEffect(() => {
		...
		if (baseOrgData && baseOrgData.contacts) {
			...
			setContactItems(
				baseOrgData.contacts.map((contact) => {
		...
		},
		{
			...
			label: (
				<div className={'sa-flex-space'}>
					<div>Контактные лица ({contactItems.length})</div>
					<div className={'sa-flex-space'}>
							...
							<div>
								{modalUsersExpanded > 0 ? (
									<Button
										...
										size={'small'}
										icon={<ChevronDoubleUpIcon height={'20px'} style={{ marginTop: '4px' }} />}
									>
										Свернуть всех
									</Button>
								) : (
									<Button
										...
										size={'small'}
										icon={<ChevronDoubleDownIcon height={'20px'} style={{ marginTop: '4px' }} />}
									>
										Развернуть всех
									</Button>
								)}
