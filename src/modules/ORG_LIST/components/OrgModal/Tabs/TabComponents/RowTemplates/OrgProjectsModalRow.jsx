...
const OrgProjectsModalRow = (props) => {
  ...
  return (
    <div className={'sa-org-row-wrapper'}>
      <div className={`sa-org-bill-tab-form  ${baseData?.deleted === 1 ? 'sa-org-section-deleted' : ''}`}>

        <OrgModalRow
            key={'rowfla00322519_' + orgId}
            titles={['Куратор', 'Дата']}
            datas={[baseData?.curator ? baseData.curator.surname + " " + baseData.curator.name + " " + baseData.curator.secondname  : ""  ,
        ...
        <OrgModalRow
            ...
            titles={['Связанное КП']}
            datas={[baseData?.linkbid_id !== 0 ? <NavLink to={'/bids/' + baseData?.linkbid_id}>Коммерческое предложение</NavLink> : "" ]}
        />
