	...
	return (
		<div className={'sa-org-row-wrapper'}>
			<div className={`sa-org-bid-row custom ${baseData?.deleted === 1 ? 'sa-org-bid-row-deleted' : ''}`}>
					...
					<div>
						<NavLink
							to={
								'/bids/' +
								baseData?.id +
								'?frompage=orgs&fromtab=o&fromview=modal&fromid=' +
								baseData?.id
							}
						>
							{baseData?.id}
				...
				<div>
					<div>{baseData?.date ? dayjs.unix(baseData.date).format('DD.MM.YYYY') : ''}</div>
				</div>
						...
						{baseData?.orguser_id ? (
							<div>
								{baseData.contactuser?.lastname +
									' ' +
									baseData.contactuser?.name +
									' ' +
									baseData.contactuser?.middlename}
							</div>
						) : (
							''
						)}
				...
				<div>
					<div>
						{baseData?.user_id ? (
							<div>
								{ShortName(
									baseData.manager?.surname,
									baseData.manager?.name,
									baseData.manager?.secondname
								)}
							</div>
						) : (
							''
