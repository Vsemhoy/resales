...
const OrgListModalOffersTab = (props) => {
	...
	useEffect(() => {
		if (props.environment === 'modal') {
			setNavigateLink(
				'/bids?type=1&company_name=' +
					orgName +
					'&type=1&frompage=orgs&fromtab=o&fromview=modal&fromid=' +
					orgId
			);
		} else {
			setNavigateLink(
				'/bids?type=1&company_name=' + orgName + '&type=1&frompage=org&fromtab=o&fromid=' + orgId
			);
		}
	}, [orgName]);
			...
			<div className={'sa-orgtab-container'}>
				...
				<Spin spinning={showLoader} delay={500}>
					<div>
						...
						{dataList.map((item) => (
							<OrgOfferModalRow org_id={orgId} data={item} />
						))}
					</div>
				</Spin>
			</div>
