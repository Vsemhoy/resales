	...
	return [
		...
		...(calls && calls.length > 0
			? calls.map((call, index) => {
					const key = call.find((call_props) => call_props.id === 'key').value;
					const date = call.find((call_props) => call_props.id === 'date').value;
					const creator =
						call.find((call_props) => call_props.id === 'creator_name').value || 'Нет данных';
					const result =
						call.find((call_props) => call_props.id === 'result').value || 'Нет данных';
					const subscriber = call.find((call_props) => call_props.id === 'subscriber').value;
					const formattedDate = dayjs(date * 1000).format('DD.MM.YYYY');
					const Label = () => {
						return (
	...
	return [
		...
		...(meetings && meetings.length > 0
			? meetings.map((meeting, index) => {
					const key = meeting.find((meeting_props) => meeting_props.id === 'key').value;
					const date = meeting.find((meeting_props) => meeting_props.id === 'date').value;
					const creator =
						meeting.find((meeting_props) => meeting_props.id === 'creator_name').value ||
						'Нет данных';
					const result =
						meeting.find((meeting_props) => meeting_props.id === 'result').value || 'Нет данных';
		...
		licenses__items.push(
			...
			...(licenses.licenses && licenses.licenses.length > 0
				? licenses.licenses.map((license, index) => {
						...
						const formattedStartDate = dayjs(start_date * 1000).format('DD.MM.YYYY');
						const formattedEndDate = dayjs(end_date * 1000).format('DD.MM.YYYY');
						const Label = () => {
							return (
								<Tooltip title={`Действие: ${formattedStartDate}-${formattedEndDate}`}>
									<div style={{ fontSize: '12px' }}>{`${name}: ${comment}`}</div>
								</Tooltip>
							);
						};
						return {
