	...
	useEffect(() => {
		...
		if (baseData?.contacts) {
			contics = baseData.contacts.map((item) => ({
				...
				classNames: 'super',
				style: {
					outline: item.deleted
						? '2px solid #ff0000a2'
						: item.command === 'create'
						? '2px solid #2196f3'
						: '0px',
					marginBottom: item.deleted ? '3px ' : item.command === 'create' ? '3px' : '0px',
				},

				label: (
					<div className={`sa-flex-space ${item.deleted ? 'sa-orgrow-deleted' : ''}`}>
						<div>
							{item.lastname} {item.name} {item.middlename}{' '}
							<span style={{ color: 'gray', fontWeight: 100 }}>({item.id})</span>
						</div>
						{editMode && (
				...
				children: (
					<div className={`${item.deleted ? 'ant-collapse-content-box-deleted' : ''}`}>
		...
		let secids = [
			{
				...
				children: (
					<OrgPage_MainTab_Common_Section
						color={'#2196f3'}
						edit_mode={editMode}
						data={baseData}
						selects={selects}
			...
			},
			{
				...
				label: (
					<div className={`sa-flex-space`}>
						<div>Информация отдела</div>
						<div></div>
					</div>
				),
				children: (
					<OrgPage_MainTab_Depart_Section
						color={'blueviolet'}
						edit_mode={editMode}
						data={baseData}
						selects={selects}
			...
			},
			{
				...
				label: (
					<div className={`sa-flex-space`}>
						<div className={`sa-flex`}>
							...
							<Badge
								count={
									baseData?.active_licenses_bo?.length
										? baseData?.active_licenses_bo?.length +
										  baseData?.active_licenses?.length +
										  baseData?.active_tolerance?.length
										: 0
								}
								color="blue"
							/>
				...
				children: baseData ? (
					<OrgPage_MainTab_Tolerance_Section
						color={'#30c97aff'}
						edit_mode={editMode}
						data={baseData}
						selects={selects}
			...
			},
			{
				...
				label: (
					<div className={`sa-flex-space`}>
						<div className={`sa-flex`}>
							Контактные лица
							<Badge count={baseData?.contacts?.length} color="blue" />
						</div>
						<div></div>
						{editMode && (
							<Button
			...
			},
			{
				...
				label: (
					<div className={`sa-flex-space`}>
						<div className={`sa-flex`}>
							Фирмы/плательщики
							<Badge count={baseData?.requisites?.length} color="blue" />
						</div>
						<div></div>
						{editMode && (
							<Button
				...
				children: (
					<OrgPage_MainTab_Payers_Section
						color={'#f0f321ff'}
						edit_mode={editMode}
						data={baseData}
						selects={selects}
