...
const BidPage = (props) => {
	...
	const fetchBidInfo = async () => {
		if (PRODMODE) {
			try {
				let response = await PROD_AXIOS_INSTANCE.get(
					`/api/sales/offers/${bidId}?_token=${CSRF_TOKEN}&from_socket=false`,
					{
						data: {},
						_token: CSRF_TOKEN,
					}
			...
			children: (
				<div className={'sa-info-list-hide-wrapper'}>
					...
					<div className={'sa-info-list-row'}>
						...
						<Input style={{ width: '100%', height: '32px' }} value={consignee} />
					</div>
					<div className={'sa-info-list-row'}>
						<div className={'sa-list-row-label'}>
							<p>Доп. оборудование</p>
						</div>
						<Input style={{ width: '100%', height: '32px' }} value={otherEquipment} />
					</div>
				</div>
			),
		},
