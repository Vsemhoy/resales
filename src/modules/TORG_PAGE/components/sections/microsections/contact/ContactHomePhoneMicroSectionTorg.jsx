...
const ContactHomePhoneMicroSectionTorg = (props) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (editMode && baseData && props.on_change) {
        ...
        props.on_change(itemId, payload, 'contact_homephone');
      }
    }, 500);
        ...
        inputs={[
          {
            ...
            input: (
              <Input
                key={`contnumber_${baseData?.id}_${orgId}`}
                value={number}
                onChange={(e) => {
          ...
          },
          {
            ...
            input: (
              <Input
                key={`contnumber_ext_${baseData?.id}_${orgId}`} // ← исправлен дубль ключа
                value={ext}
                type="text" // ← лучше "text", т.к. добавочные могут содержать символы
                onChange={(e) => {
        ...
        extratext={[
          {
            ...
            input: (
              <TextArea
                key={`contnumber_2_${baseData?.id}_${orgId}`}
                value={comment}
                onChange={(e) => {
...
export default ContactHomePhoneMicroSectionTorg;