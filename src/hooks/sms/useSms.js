...
/**
 * useSms с polling для обновления сообщений
 */
export function useSms({ chatId = null, mock = null, enablePolling = true }) {
	...
	useEffect(() => {
		...
		// Polling: обновляем каждые 10 секунд если включено
		let intervalId;
		if (enablePolling) {
			intervalId = setInterval(() => {
				console.log('[useSms] Polling: обновление сообщений...');
				loadData(signal);
			}, 10000); // 10 секунд
		}
